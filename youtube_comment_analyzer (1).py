...
# 한글 폰트 자동 다운로드 & 탐색
# -----------------------------
# Streamlit Cloud 포함 어떤 환경에서도 동작하도록 절대경로 사용
BASE_DIR = os.path.dirname(os.path.abspath(__file__))
FONT_PATH = os.path.join(BASE_DIR, "NanumGothic.ttf")

def ensure_korean_font():
    # 1) 스크립트 옆에 폰트 파일이 이미 있으면 바로 사용
...
KOREAN_FONT = ensure_korean_font()
...
def create_wordcloud(text):
    ...
    if not KOREAN_FONT or not os.path.exists(KOREAN_FONT):
        st.error("한글 폰트를 불러올 수 없습니다. NanumGothic.ttf 파일을 스크립트와 같은 폴더에 넣어주세요.")
        return None
    return WordCloud(
...
if st.button("댓글 분석 시작"):
    ...
    sns.histplot(df["likes"], bins=20, kde=True, ax=ax2)
    st.pyplot(fig2)
    ...
    st.dataframe(top_comments[["comment", "likes"]])
